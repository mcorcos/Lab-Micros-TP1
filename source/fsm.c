 ...
 */
int verify_pin(uint8_t data[], int user_num);
        ...
        if (!timerRunning(timer)) {
            ...
            if (timerExpired(timer)) {
                ...
                if (magtek_inter || encoder_inter == BUTTON) {
                    pre_state = state;
                    state = PRE_ID;
                    //Capaz q aca hay que apagar los inter flags pero fijarse :)
                    timerStop(timer);
                }
...
uint8_t get_number(void) {
	if (encoder.cantGiros >= 0) {
		if (encoder.cantGiros >= 10) {
			return encoder.cantGiros % 10;
		}
		else {
			return encoder.cantGiros;
		}
	}
	else {
		if (encoder.cantGiros <= -10) {
			return 10 + encoder.cantGiros % 10;
		}
		else {
			return 10 + encoder.cantGiros;
		}
	}
}
