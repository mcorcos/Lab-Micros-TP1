                ...
                if (encoder_inter) {
					if(encoder.cantPresion == BRIGHTNESS){
                        ...
                        if (encoder.cantPresion == OK) {
                            ...
                            temp_id[i] = data.id[i]; //VER SI ESTO ES '-' O EL NUMERO!!
                            ++i;
                            clearEncoderPresiones();
                            clearEncoderGiros();
                            temp_id[i] = '0';
        ...
        if (!timerRunning(timer)) {
            ...
            if (timerExpired(timer)) {
                ...
                if (encoder_inter) {
                    ...
                    encoder_inter = clearEncoderInter();
					temp_pin[i] = get_number(); // ACA ES DONDE IMPRIME DONDE GIRA
                }
                if(!waiting_pin) {
                    pre_state_fsm = state_fsm;
                    state_fsm = PIN;
